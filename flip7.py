...
class Flip7(Game):
    ...
    def get_observation(self) -> Observation:
        ...
        # bust_probabilities = [
        #     Flip7.bustProbability(p, self.deck, self.nrPlayersStillPlaying)
        #     for p in self.players
        # ]
        bust_probability = Flip7.bustProbability(
            self.activePlayer, self.deck, self.nrPlayersStillPlaying
        )

        handScore = (
            self.getPlayerHandScore(self.activePlayer) if self.activePlayer else None
        )
    ...
    @staticmethod
    def matchProbability(player: Player, deck: Deck):
        """
    ...
    def bustProbability(player: Player, deck: Deck, nrPlayersStillPlaying=2):
        ...
        if Flip7.FLIP_THREE in deck and nrPlayersStillPlaying == 1:
            matchByFlipThree = Flip7.matchByFlipThreeProbability(player, deck)
        else:
    ...
    @staticmethod
    def matchByFlipThreeProbability(player: Player, deck: Deck):
        ...
        p_single = Flip7.matchProbability(player, deck)
        q_single = 1 - p_single
...
class simpleRisk(Strategy):
    """Simple strategy that selects uniformly at random among legal actions."""

    def __init__(self, riskFactor=0.25):
        super().__init__()
        # This strategy draws until the chance to bust is larger
    ...
    def choose_action(self, obs: Observation, legal_actions: list[Action]) -> Action:
        ...
        if obs.phase == Flip7.PHASE_EFFECT_CHOOSE:
            scores = [a.target_player.score for a in legal_actions]
            scores.sort()
            if obs.actingPlayer.score == scores[-1] and len(scores) > 1:
                targetScore = scores[-2]
            else:
                targetScore = scores[-1]

            for action in legal_actions:
                if (
                    action.target_player.score == targetScore
                    and action.target_player != action.acting_player
                ):
                    return action
            raise RuntimeError("No valid target found!")
        elif obs.phase == Flip7.PHASE_FLIP:
            bust_prob = obs.extras["bust_probability"]
            for a in legal_actions:
                if a.type == ActionType.DRAW and bust_prob <= self.riskFactor:
                    return a
                elif a.type == ActionType.PASS and bust_prob > self.riskFactor:
                    return a
            raise RuntimeError("No choice made! Faulty strategy!")


class simpleRiskEstimator(Strategy):
    """Simple strategy that selects uniformly at random among legal actions."""

    def __init__(self, riskFactor=25):
        super().__init__()
        # This strategy draws until the player has reached a specific score
        self.riskFactor = riskFactor
    ...
    def choose_action(self, obs: Observation, legal_actions: list[Action]) -> Action:
        ...
        if obs.phase == Flip7.PHASE_EFFECT_CHOOSE:
            ...
            handScore = obs.extras["hand_score"]
            for a in legal_actions:
                if a.type == ActionType.DRAW and handScore <= self.riskFactor:
                    return a
                elif a.type == ActionType.PASS and handScore > self.riskFactor:
                    return a
            raise RuntimeError("No choice made! Faulty strategy!")
